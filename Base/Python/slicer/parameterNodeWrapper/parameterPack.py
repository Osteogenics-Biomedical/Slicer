...
def _makeConcreteProperty(name):
    return property(
...
def _initMethod(self, *args, **kwargs):
...
class ObservedParameterPack:
    ...
    """
    def __init__(self, parameterNode, serializer, name, startingValue):
        # Because we are overriding __getattr__ and __setattr__ to pass through to _value
        # we need to access this class's members directly through __dict__
        self.__dict__['_parameterNode'] = parameterNode
        self.__dict__['_serializer'] = serializer
        self.__dict__['_name'] = name
        self.__dict__['_value'] = startingValue

    def __str__(self) -> str:
        return f"Observed({str(self.__dict__['_value'])})"

    def __repr__(self) -> str:
        return str(self)

    def __getattr__(self, name):
        myValue = self.__dict__['_value']
    ...
    def __setattr__(self, name: str, value) -> None:
        ...
        if hasattr(myValue, name):
            ...
            serializer = self.__dict__['_serializer']
            raise AttributeError(f"'{str(self._serializer.type)}' has no attribute '{name}'"
                                 " and attributes cannot be added dynamcially")

    def _save(self) -> None:
        serializer = self.__dict__['_serializer']
        parameterNode = self.__dict__['_parameterNode']
        ...
        try:
            ...
            # parameterPacks, etc)
            self.__dict__['_value'] = serializer.read(parameterNode, name).__dict__['_value']
    ...
    def __eq__(self, other) -> bool:
        if isinstance(other, ObservedParameterPack) and self._serializer.type == other._serializer.type:
            ...
            return self.__dict__['_value'] == other
...
class _ParameterPackInstanceValidator(Validator):
    ...
    def validate(self, value):
        checkValue = value.__dict__['_value'] if isinstance(value, ObservedParameterPack) else value
...
class ParameterPackSerializer(Serializer):
    ...
    def _serializer(self, parameterInfo):
        return _getSerializer(self.type, parameterInfo.basename)

    def _mangleName(self, parameterInfo, name):
        return f"{name}_{parameterInfo.basename}"
